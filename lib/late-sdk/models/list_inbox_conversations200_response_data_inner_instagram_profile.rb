...
module Late
  ...
  class ListInboxConversations200ResponseDataInnerInstagramProfile < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Late::ListInboxConversations200ResponseDataInnerInstagramProfile` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Late::ListInboxConversations200ResponseDataInnerInstagramProfile`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [is_follower, is_following, follower_count, is_verified, fetched_at].hash
    end
