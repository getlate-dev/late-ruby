...
module Late
  class LogsApi
    ...
    def get_log(log_id, opts = {})
      data, _status_code, _headers = get_log_with_http_info(log_id, opts)
      data
    end
    ...
    # @param log_id [String] The log entry ID
    # @param [Hash] opts the optional parameters
    # @return [Array<(GetLog200Response, Integer, Hash)>] GetLog200Response data, response status code and response headers
    def get_log_with_http_info(log_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: LogsApi.get_log ...'
      end
      # verify the required parameter 'log_id' is set
      if @api_client.config.client_side_validation && log_id.nil?
        fail ArgumentError, "Missing the required parameter 'log_id' when calling LogsApi.get_log"
      end
      # resource path
      local_var_path = '/v1/logs/{logId}'.sub('{' + 'logId' + '}', CGI.escape(log_id.to_s))
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'GetLog200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['bearerAuth']

      new_options = opts.merge(
        :operation => :"LogsApi.get_log",
      ...
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: LogsApi#get_log\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
    ...
    def get_post_logs(post_id, opts = {})
      data, _status_code, _headers = get_post_logs_with_http_info(post_id, opts)
      data
    end
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Integer] :limit Maximum number of logs to return (max 100) (default to 50)
    # @return [Array<(GetPostLogs200Response, Integer, Hash)>] GetPostLogs200Response data, response status code and response headers
    def get_post_logs_with_http_info(post_id, opts = {})
      if @api_client.config.debugging
        @api_client.config.logger.debug 'Calling API: LogsApi.get_post_logs ...'
      end
      # verify the required parameter 'post_id' is set
      if @api_client.config.client_side_validation && post_id.nil?
        fail ArgumentError, "Missing the required parameter 'post_id' when calling LogsApi.get_post_logs"
      end
      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] > 100
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling LogsApi.get_post_logs, must be smaller than or equal to 100.'
      end

      if @api_client.config.client_side_validation && !opts[:'limit'].nil? && opts[:'limit'] < 1
        fail ArgumentError, 'invalid value for "opts[:"limit"]" when calling LogsApi.get_post_logs, must be greater than or equal to 1.'
      end

      # resource path
      local_var_path = '/v1/posts/{postId}/logs'.sub('{' + 'postId' + '}', CGI.escape(post_id.to_s))
      ...
      # return_type
      return_type = opts[:debug_return_type] || 'GetPostLogs200Response'

      # auth_names
      auth_names = opts[:debug_auth_names] || ['bearerAuth']

      new_options = opts.merge(
        :operation => :"LogsApi.get_post_logs",
        :header_params => header_params,
        :query_params => query_params,
        :form_params => form_params,
        :body => post_body,
      ...
      data, status_code, headers = @api_client.call_api(:GET, local_var_path, new_options)
      if @api_client.config.debugging
        @api_client.config.logger.debug "API called: LogsApi#get_post_logs\nData: #{data.inspect}\nStatus code: #{status_code}\nHeaders: #{headers}"
      end
      return data, status_code, headers
    end
    ...
    def list_logs_with_http_info(opts = {})
      ...
      if @api_client.config.client_side_validation && !opts[:'days'].nil? && opts[:'days'] > 7
        fail ArgumentError, 'invalid value for "opts[:"days"]" when calling LogsApi.list_logs, must be smaller than or equal to 7.'
      end
